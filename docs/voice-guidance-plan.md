...
### 4.2 The rewrite rules

Applied in order to `instruction`, producing the spoken string. The on-screen
banner keeps the original — these changes are for the ear only.

1. `": "` → `", "`. A colon is a sign convention, not speech. *"Take the exit
   toward I 93 South, Quincy."*
2. `" / "` → `" and "`. *"toward Soldiers Field Road West and Newton"*,
   *"toward I 93 South and US 1 South"*.
3. Road refs: `I 93` and `I-93` → `Interstate 93`; `US 1` → `Route 1`;
   `MA 60`, `RI 1A`, `NH 101`, `VT 100`, `ME 26`, `CT 15` → `Route 60` and so
   on. Massachusetts signs and Massachusetts drivers both say "Route 60"; the
   two-letter prefix is a database artefact. Applied by regex on
   `\b(?:I|US|MA|RI|NH|VT|ME|CT)[ -](\d+[A-Z]?)\b`, which is why the state
   prefixes are listed exhaustively rather than matched as any two capitals —
   `YMCA Drive` must not become `Route CA`.
4. Numeric ordinals → words: `1st` → `first` … `9th` → `ninth`. Generated by
   `_ordinal()` in `pipeline/router.py:1349`; rewriting on the phone rather than
   at the source keeps the screen reading `2nd`, which is right for the screen.
5. Leave `Take exit 14` alone. Bare cardinals are read correctly and *"exit
   fourteen"* is what a driver wants.

### 4.3 The exhaustive known-bad list

Every instruction in the corpus containing a digit, colon or slash, with what it
becomes:
...
| `Take the 1st exit at Kosciuszko Circle onto William T. Morrissey Boulevard` | Take the first exit at Kosciuszko Circle onto William T. Morrissey Boulevard |
| `Take the 2nd exit at Middleborough Rotary onto Bedford Street` | Take the second exit at Middleborough Rotary onto Bedford Street |
| (and six more `Take the {1st,2nd} exit …`, same rule) | |
| `Take exit 9 toward West Quincy` | unchanged |
| `Turn left onto YMCA Drive` | unchanged — see below |

**Unverified.** These rewrites are chosen from what the strings contain, and the
frequencies above are measured, but *how the synthesiser pronounces the
originals was not measured* — there is no way to hear the phone from here and no
offline transcriber on this Mac. `MA 60` may already be read acceptably; `YMCA`
may already be spelled out. Settled by listening to the 18 strings once, which
takes about a minute with the spike on the branch, or by a
synthesise-then-recognise round trip. Until then rule 3 is the one to be
sceptical of, and it is also the one with the largest blast radius, since it
touches any name that looks like a state prefix.
...
using `roundabout_exit` and dropping the duplicated name. That is a §4.2 rule 6
if listening confirms it, not a reason to abandon the normalised sentence.
...
| stage | what | estimate |
|---|---|---|
| **0** | Add `audio` to `UIBackgroundModes` in `ios/project.yml`, `xcodegen generate`. | 10 min |
| **1** | `Speaker` (session + synthesiser + pre-emption + interruptions) and `VoiceGuide` (schedule, latch, `pace`), injected into `NavigationModel`; final utterance only, normalised instruction, arrival announcement. | 1 day |
| **2** | Prepare utterance, `prepareFloor`, chaining, the `continue`-gets-no-prepare rule. | ½ day |
| **3** | Mute glyph in the banner, `UserDefaults` persistence. | 2–3 h |
| **4** | Phrasing normaliser for §4.2, after listening to the 18 strings in §4.3. | ½ day |
| **5** | `actionProblem` reporting for session failures (2 h), and an ndjson replay harness plus the latch tests over the five recorded drives — **the harness does not exist yet**, see §10 (1 day). | 1½ days |
