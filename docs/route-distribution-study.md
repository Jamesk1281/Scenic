...
21 minutes, one process, seed 20260829 recorded in the output. `data/` and
`.venv` live in the main checkout, not in a worktree, so run it from there or
pass absolute paths. Re-cut any threshold from the CSVs without re-routing:
